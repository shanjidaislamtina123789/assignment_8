...
export default function Register({ regName, setRegName, regEmail, setRegEmail, regPhoto, setRegPhoto, regPassword, setRegPassword, handleRegister, navigateTo }) {
    ...
    <section className="px-4 py-16 max-w-md mx-auto">
      <div className="bg-white p-8 rounded-3xl border border-stone-100 shadow-xl space-y-6">
        <div className="text-center space-y-1">
          <h1 className="font-serif text-2xl font-black text-stone-900">Create Account </h1>
        </div>
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-stone-600 uppercase tracking-wider mb-1">Full Legal Name</label>
            <input type="text" required placeholder="Name" value={regName} onChange={(e) => setRegName(e.target.value)} className="w-full bg-stone-50 border border-stone-200 focus:border-emerald-800 rounded-xl p-3 text-xs font-medium text-stone-800 focus:outline-none" />
          </div>
          <div>
            <label className="block text-xs font-bold text-stone-600 uppercase tracking-wider mb-1">Email Account</label>
            <input type="email" required placeholder="email@gmail.com" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} className="w-full bg-stone-50 border border-stone-200 focus:border-emerald-800 rounded-xl p-3 text-xs font-medium text-stone-800 focus:outline-none" />
          </div>
          <div>
          ...
          <button type="submit" className="w-full bg-emerald-800 hover:bg-emerald-900 text-white font-bold text-xs uppercase tracking-widest py-3.5 rounded-xl transition-colors shadow-md">Register</button>
        </form>
        ...
        <p className="text-center text-xs text-stone-500 font-medium pt-2">Already registered? <button type="button" onClick={() => navigateTo('login')} className="text-emerald-800 font-bold hover:underline">Login Securely</button></p>
      </div>
    </section>
  );
}